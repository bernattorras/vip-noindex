...
class VIPNonProductionIndexing {
	...
	/**
	 * Determine if the current site should be indexed or not and proceed accordingly.
	 *
	 * @return void
	 */
	public function init() {
		...
		if ( 'production' !== VIP_GO_APP_ENVIRONMENT || false !== strpos( $_SERVER['HTTP_HOST'] ?? '', '.go-vip.' )
			...
			add_action( 'admin_notices', [ $this, 'admin_indexable_notice' ] );

			// Disable the 'blog_public' checkbox.
			add_action( 'admin_footer', [ $this, 'disable_blog_public_option' ] );

			// Disalow site in robots.txt.
			add_action( 'do_robotstxt', [ $this, 'disallow_all_user_agents' ] );
		}
	}
	 ...
	 * @return void
	 */
	public function disallow_all_user_agents() {
	 ...
	 * @return void
	 */
	public function admin_indexable_notice() {
		?>
	 ...
	 * @return void
	 */
	public function disable_blog_public_option() {
...
$wpdiu = new VIPNonProductionIndexing();
